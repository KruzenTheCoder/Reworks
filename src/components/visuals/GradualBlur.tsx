...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = props => {
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const safeZIndex =
      typeof config.zIndex === 'number' && !Number.isNaN(config.zIndex)
        ? config.zIndex
        : (DEFAULT_CONFIG.zIndex as number);
    ...
    const baseStyle: CSSProperties = {
      ...
      zIndex: isPageTarget ? safeZIndex + 100 : safeZIndex,
      ...config.style
    };
